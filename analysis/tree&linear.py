...
#读取数据
def read_x_y():
    train = pd.read_csv("H:\kaggle\houseprice\data\Mtrain.csv")
    test = pd.read_csv("H:\kaggle\houseprice\data\Mtest.csv")
    del test['SalePrice']
    #提取训练集中的xy
...
if __name__ == '__main__':
    ...
    RForest = ensemble.RandomForestRegressor(random_state=14,n_jobs=-1)
    ...
    pd.DataFrame({'Id':test_Id,'SalePrice':final}).to_csv('H:/kaggle/houseprice/data/result.csv',index = False)
